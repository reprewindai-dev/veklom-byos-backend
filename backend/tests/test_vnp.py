...
def test_vnp_ingestion_invalid_validator():
    ...
    with patch("backend.apps.api.routers.vnp.AsyncSession.get", new_callable=AsyncMock) as mock_get:
...
def test_vnp_ingestion_valid_signature():
    ...
    message = f"api-openai-com:valid-val:us-east-1:150:200:1".encode('utf-8')
    signed = signing_key.sign(message, encoder=HexEncoder)
    signature_hex = signed.signature.decode('utf-8')
    
    with patch("backend.apps.api.routers.vnp.AsyncSession.get", new_callable=AsyncMock) as mock_get, \
         patch("backend.apps.api.routers.vnp.update_api_composite_score", new_callable=AsyncMock) as mock_update:
        mock_get.return_value = mock_validator
        mock_update.return_value = 99.5
        
        # Also need to mock commit and add on db
        with patch("backend.apps.api.routers.vnp.AsyncSession.commit", new_callable=AsyncMock), \
...
def test_vnp_beacon_all():
    ...
    with patch("backend.apps.api.routers.vnp.AsyncSession.execute", new_callable=AsyncMock, side_effect=mock_execute):
        response = client.get("/api/vnp/beacon")
...
def test_vnp_beacon_single():
    ...
    with patch("backend.apps.api.routers.vnp.get_cached_api_score", new_callable=AsyncMock) as mock_cache:
        mock_cache.return_value = {"score": 95.0, "rating": "Stable", "updated_at": "2024-01-01T00:00:00Z"}
        
        response = client.get("/api/vnp/beacon?api_id=test-api")
