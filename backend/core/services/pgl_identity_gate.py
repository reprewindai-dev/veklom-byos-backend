...
class PGLIdentityError(Exception):
    ...
    def __init__(self, actor_id: str, reason: str):
        ...
        self.reason = reason
        super().__init__(f"[PGL GATE] Execution BLOCKED for agent '{actor_id}': {reason}")
...
class PGLIdentityGate:
    ...
    async def _resolve_or_seed_system_agent(
        ...
        result = await db.execute(select(PGLIdentity).where(PGLIdentity.id == actor_id))
        ...
        try:
            ...
            await redis_client.set(f"veklom:pgl:identity:{identity.id}", json.dumps(cache_payload), ex=3600)
        except Exception as e:
    ...
    async def _resolve_registered_agent(
        ...
        agent_result = await db.execute(select(Agent).where(Agent.agent_id == actor_id))
        ...
        cert_result = await db.execute(select(BirthCertificate).where(BirthCertificate.agent_id == agent.id))
        ...
        result = await db.execute(select(PGLIdentity).where(PGLIdentity.id == cert.pgl_identity_id))
        identity = result.scalar_one_or_none()
        ...
        if identity:
            try:
                ...
                if actor_id != identity.id:
                    await redis_client.set(f"veklom:pgl:identity:{actor_id}", json.dumps(cache_payload), ex=3600)
            except Exception as e:
    ...
    def _check_status(identity: "PGLIdentity", actor_id: str) -> None:  # noqa: F821
        ...
        if status == "SUSPENDED":
            raise PGLIdentitySuspended(
                actor_id=actor_id, reason=f"Agent is SUSPENDED: {meta.get('suspension_reason', 'No reason recorded')}."
            )
        if status in ("REVOKED", "DEACTIVATED", "DELETED"):
            raise PGLIdentityError(
                actor_id=actor_id, reason=f"Agent identity status is '{status}' — permanently blocked from execution."
            )
    ...
    async def require(
        ...
        """
        logger.info(f"[PGLGate] 🔐 Identity check — actor='{actor_id}' action='{action}'")

        # ── 1. Resolve identity ───────────────────────────────────────────────
        try:
            if kind == AgentKind.SYSTEM and actor_id in SYSTEM_AGENT_CONFIGS:
                config = SYSTEM_AGENT_CONFIGS[actor_id]
                identity = await PGLIdentityGate._resolve_or_seed_system_agent(db, actor_id, config)
            ...
            raise PGLIdentityError(actor_id=actor_id, reason=f"Identity resolution failed unexpectedly: {exc}") from exc
        ...
        try:
            ...
            # ⚡ Bolt: Consolidated two queries into a single query using conditional aggregation.
            # Pushes the count to the database instead of loading all rows into Python memory with .scalars().all().
            stats_result = await db.execute(
                select(
                    func.sum(case((PGLCertificate.status == "SUCCEEDED", 1), else_=0)).label("attestations"),
                    func.sum(case((PGLCertificate.status.in_(["FAILED", "ROLLED_BACK"]), 1), else_=0)).label(
                        "rollbacks"
                    ),
                ).where(
            ...
            active_attestations = int(getattr(stats_row, "attestations", 0) or 0) if stats_row else 0
            active_rollbacks = int(getattr(stats_row, "rollbacks", 0) or 0) if stats_row else 0
            ...
            if _lc.trust_level == TrustLevel.HARD_EXPIRED:
                ...
                raise PGLIdentityExpired(actor_id=actor_id, reason=n.message, notification_dict=_lifecycle_notification)
                ...
                n = notify_renewal_due(
                    pgl_id=pgl_id, days_remaining=_lc.days_until_renewal, renewal_due_at=_lc.renewal_due_at.isoformat()
                )
                _lifecycle_notification = n.to_dict()
                _lifecycle_warning = n.message
        ...
        try:
            ...
            raise PGLCertificateError(
                actor_id=actor_id, reason=f"Could not issue pre-execution PGL certificate: {exc}"
            ) from exc
        ...
        try:
            ...
            agent_res = await db.execute(select(Agent).where(Agent.agent_id == actor_id))
            agent_row = agent_res.scalar_one_or_none()
            if agent_row:
                cert_res = await db.execute(select(BirthCertificate).where(BirthCertificate.agent_id == agent_row.id))
    ...
    async def attest_success(
        ...
        try:
            ...
            logger.error(
                f"[PGLGate] ⚠️  attest_success failed for '{ctx.actor_id}' (action completed, certificate gap): {exc}"
            )
    ...
    async def attest_failure(
        ...
        try:
            ...
            logger.error(f"[PGLGate] Failed to register PGL rollback for '{ctx.actor_id}': {exc}")
...
async def require_pgl_identity(
    ...
    """
    return await PGLIdentityGate.require(db=db, actor_id=actor_id, action=action, payload=payload, kind=kind)
