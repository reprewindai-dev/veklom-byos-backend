"""CAPI router – governs CAPI fee metering.

Service boundary
----------------
This router computes the deterministic idempotency key and delegates **all**
fee persistence to :class:`SettlementLedgerRepository`.  No direct
``SettlementLedger`` writes occur here.
"""

from __future__ import annotations

import hashlib
from typing import Optional

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.ext.asyncio import AsyncSession

from backend.core.database.database import get_db
from backend.db.repositories.settlement_repo import SettlementLedgerRepository

router = APIRouter(prefix="/capi", tags=["capi"])


def _build_idempotency_key(
    tenant_id: str,
    execution_id: str,
    fee_type: str,
    provider: str,
) -> str:
    """Derive a deterministic idempotency key.

    Identical algorithm to the one in identity_rag.py so that cross-router
    fee keys share the same namespace and collision properties.
    """
    raw = f"{tenant_id}:{execution_id}:{fee_type}:{provider}"
    return hashlib.sha256(raw.encode()).hexdigest()[:64]


@router.post("/fee", status_code=status.HTTP_201_CREATED)
async def record_capi_fee(
    *,
    db: AsyncSession = Depends(get_db),
    tenant_id: str,
    execution_id: str,
    provider: str = "veklom",
    fee_type: str = "capi",
    amount: int,
    currency: str = "USDC",
    authority_run_id: Optional[str] = None,
    payment_proof_id: Optional[str] = None,
) -> dict:
    """Record a metered fee for a CAPI execution.

    Idempotent: replaying the same (tenant_id, execution_id, fee_type,
    provider) combination is safe and returns the existing row.
    """
    idempotency_key = _build_idempotency_key(
        tenant_id=tenant_id,
        execution_id=execution_id,
        fee_type=fee_type,
        provider=provider,
    )

    repo = SettlementLedgerRepository(db)
    row = await repo.create_fee_entry(
        tenant_id=tenant_id,
        provider=provider,
        fee_type=fee_type,
        amount=amount,
        currency=currency,
        idempotency_key=idempotency_key,
        execution_id=execution_id,
        authority_run_id=authority_run_id,
        payment_proof_id=payment_proof_id,
    )
    await db.commit()

    return {
        "ledger_id": str(row.id),
        "idempotency_key": row.idempotency_key,
        "status": row.status.value,
        "amount": row.amount,
        "currency": row.currency,
    }
