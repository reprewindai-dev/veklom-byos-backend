...
## 2024-08-25 - Fetching Tuples Instead of ORM Models
**Learning:** Using `.scalars().all()` on SQLAlchemy queries for large datasets loads the full ORM models into memory, leading to a significant instantiation overhead. This causes memory and execution time bottlenecks on endpoints fetching lists of rows.
**Action:** When querying for read-only aggregation lists (like `recent_runs` or `autonomous_decisions`), select specific columns via `select(Model.col1, Model.col2)` and retrieve them as lightweight tuple rows using `.all()` (or `.fetchall()`), which bypasses the expensive ORM model instantiation process.
