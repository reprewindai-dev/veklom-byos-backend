...
from backend.compliance.law25_compliance_framework import ProcessingPurpose as Law25Purpose
...
class MultiJurisdictionEnforcer:
    ...
    def auto_enforce_consent(
        ...
        if LegalFramework.LAW25 in self.active_frameworks:
            try:
                law25_purpose = Law25Purpose(processing_purpose)
            except ValueError:
                results.append((False, f"Law 25: Unsupported processing purpose {processing_purpose}"))
            else:
                has_consent = self.law25_framework.check_consent(
                    user_id=user_id,
                    purpose=law25_purpose,
                )
                if not has_consent:
                    msg = f"Law 25: No consent for {processing_purpose}"
                    results.append((False, msg))
                else:
                    results.append((True, f"Law 25: Consent verified for {processing_purpose}"))
        
        # Check GDPR (if applicable)
        if LegalFramework.GDPR in self.active_frameworks:
            has_consent = any(
                c.data_subject_id == user_id
                and c.legal_basis == GDPRBasis.CONSENT
                and c.purpose == processing_purpose
                and (c.expires_at is None or c.expires_at > datetime.utcnow())
                for c in self.gdpr_framework.consent_records.values()
            )
            if has_consent:
                results.append((True, "GDPR: Consent verified"))
            else:
                results.append((False, "GDPR: Explicit consent required and no active consent record exists"))
    ...
    def _check_ccpa_opt_out(self, user_id: str) -> bool:
        ...
        return self.ccpa_framework.has_opted_out(user_id)
