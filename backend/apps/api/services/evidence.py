...
from backend.core.replay.ingestion import CloudEvent, replay_ingestion
...
class EvidencePackBuilder:
    ...
    async def build_evidence_pack(
        ...
        run_result = await self.db.execute(select(AuthorityRun).where(AuthorityRun.id == authority_run_id))
    ...
    async def _collect_artifacts(self, run: AuthorityRun) -> Dict[str, Any]:
        ...
        agent_result = await self.db.execute(select(Agent).where(Agent.agent_id == run.agent_id))
        agent = agent_result.scalar_one_or_none()

        if agent:
            cert_result = await self.db.execute(select(BirthCertificate).where(BirthCertificate.agent_id == agent.id))
            cert = cert_result.scalar_one_or_none()
            if cert:
                artifacts["birth_certificate_id"] = cert.certificate_id

        # Get memory entries for this run
        memory_result = await self.db.execute(select(MemoryEntry).where(MemoryEntry.authority_run_id == run.id))
        ...
        browser_result = await self.db.execute(select(BrowserAction).where(BrowserAction.authority_run_id == run.id))
...
class EvidencePackVerifier:
    ...
    async def verify_evidence_pack(self, evidence_pack_id: str) -> Dict[str, Any]:
        ...
        # Get the authority run
        run_result = await self.db.execute(select(AuthorityRun).where(AuthorityRun.id == pack.authority_run_id))
        run = run_result.scalar_one_or_none()
        ...
        for hash_type in ["input_hash", "output_hash", "audit_hash", "ledger_hash", "artifacts_hash"]:
            if recomputed_hashes[hash_type] != original_hashes.get(hash_type):
                failures.append(
                    f"{hash_type} mismatch: expected {original_hashes.get(hash_type)}, got {recomputed_hashes[hash_type]}"
                )
        ...
        return {
            ...
            "hash_comparison": {"original": original_hashes, "recomputed": recomputed_hashes},
        }
    ...
    async def _verify_artifacts(self, artifacts: Dict[str, Any]) -> List[str]:
        ...
        memory_ids = artifacts.get("memory_entry_ids", [])
        if memory_ids:
            memory_result = await self.db.execute(
                select(MemoryEntry.memory_entry_id).where(MemoryEntry.memory_entry_id.in_(memory_ids))
            )
            found_memory_ids = set(memory_result.scalars().all())
            for memory_id in memory_ids:
                if memory_id not in found_memory_ids:
                    failures.append(f"Memory entry {memory_id} not found")

        # Verify browser actions
        action_ids = artifacts.get("browser_action_ids", [])
        if action_ids:
            action_result = await self.db.execute(
                select(BrowserAction.browser_action_id).where(BrowserAction.browser_action_id.in_(action_ids))
            )
            found_action_ids = set(action_result.scalars().all())
            for action_id in action_ids:
                if action_id not in found_action_ids:
                    failures.append(f"Browser action {action_id} not found")

        # Verify ledger events
        event_ids = artifacts.get("ledger_event_ids", [])
        if event_ids:
            event_result = await self.db.execute(select(LedgerEvent.id).where(LedgerEvent.id.in_(event_ids)))
            found_event_ids = set(event_result.scalars().all())
...
class EvidenceService:
    ...
    async def get_evidence_pack(self, evidence_pack_id: str) -> Optional[EvidencePack]:
        ...
        result = await self.db.execute(select(EvidencePack).where(EvidencePack.evidence_pack_id == evidence_pack_id))
