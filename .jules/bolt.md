...
## 2026-08-14 - Batch boolean counts in SQLAlchemy
**Learning:** Multiple scalar queries over the same base relation can be collapsed into one roundtrip with conditional aggregates such as `func.sum(case(...))` while preserving endpoint semantics.
**Action:** When monitoring endpoints count multiple sub-conditions over the same workspace/time window, prefer one `select(...)` containing the required aggregates and keep regression coverage for empty/default and error-rate behavior.
