...
CODE=$(curl -s -o /dev/null -w '%{http_code}' http://localhost:80/api/v1/auth/me)
[ "$CODE" = "401" ] && echo "PASS: $CODE" || echo "FAIL: $CODE"

echo ""
echo "=== Test 2: Demo token (expect 401) ==="
CODE=$(curl -s -o /dev/null -w '%{http_code}' -H 'Authorization: Bearer veklom-demo-token-quantum' http://localhost:80/api/v1/auth/me)
[ "$CODE" = "401" ] && echo "PASS: $CODE" || echo "FAIL: $CODE"

echo ""
echo "=== Test 3: Eval session (expect 200) ==="
RESP=$(curl -s -X POST http://localhost:80/api/v1/auth/eval-session \
...
curl -s -H "Authorization: Bearer $TOKEN" http://localhost:80/api/v1/auth/me | python3 -c "
import sys, json
...
curl -s -H "Authorization: Bearer $TOKEN" http://localhost:80/api/v1/workspace/overview | python3 -c "
import sys, json
d = json.load(sys.stdin)
