...
async def _overview_payload(db: AsyncSession, workspace_id: str, actor_email: str):
    ...
    # Optimization: Fetching specific columns instead of full ORM objects reduces memory and latency.
    model_rows = (await db.execute(
        select(ModelConfig.id, ModelConfig.provider, ModelConfig.display_name)
        .where(ModelConfig.workspace_id == workspace_id, ModelConfig.is_enabled == True)
    )).all()
    model_payload = [
    ...
    # Optimization: Fetch specific columns to avoid N+1 ORM object instantiation overhead.
    result = await db.execute(
        select(
    ...
    # Optimization: Fetching specific columns directly eliminates full object parsing.
    audit_rows = (await db.execute(
        select(
            AuditLog.id, AuditLog.action, AuditLog.resource_type, AuditLog.resource_id,
            AuditLog.user_id, AuditLog.hash_chain, AuditLog.prev_hash, AuditLog.created_at
        )
    ...
    try:
        # Optimization: Tuple unpack prevents loading overhead in fast dashboard renders.
        alert_rows = (await db.execute(
            select(
                SecurityEvent.id, SecurityEvent.description, SecurityEvent.event_type,
                SecurityEvent.severity, SecurityEvent.threat_type, SecurityEvent.created_at
            )
