        ...
        try:
            ...
            if not account_id or account_id.startswith("acct_mock_"):
                account = stripe.Account.create(
                    type="express",
                    country="US",
                    email=getattr(user, "email", "support@veklom.com"),
                    capabilities={
            ...
            # Generate Account Link via standard Stripe V1 API
            account_link = stripe.AccountLink.create(
                account=account_id,
                type="account_onboarding",
                refresh_url=f"https://veklom.com/api/v1/x402/onboarding/callback?status=cancelled&account_id={account_id}&user_id={user.id}",
                return_url=f"https://veklom.com/api/v1/x402/onboarding/callback?status=success&account_id={account_id}&user_id={user.id}"
            )
