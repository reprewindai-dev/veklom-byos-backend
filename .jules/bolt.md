...
## 2024-07-24 - Async File I/O Optimization in Async Contexts
**Learning:** Using synchronous `open()` and `json.dump()`/`json.load()` within an asynchronous function blocks the main event loop, significantly increasing response times and reducing concurrency throughput.
**Action:** Always utilize `aiofiles` in conjunction with `json.dumps()`/`json.loads()` when performing file I/O operations inside `async` routines to prevent event loop blocking. Ensure proper usage by prefixing operations with `await` and avoiding modifications to lockfiles unless strictly necessary.
