...
from backend.db.models.mission_lock import (
    Base, MissionDNA, AgentMission, MissionLockAgentState, AgentRuntimeState,
    TenantRole, IdempotencyKey, EpisodeTelemetry
)
from backend.core.services.mission_lock_service import (
    MissionLockService, TabularPolicy, MissionPath, MissionLockAgent
)
...
async def test_tabular_policy_serialization_deserialization():
    ...
    policy.update(state="idle", action="fetch_task", reward=1.0, next_state="task_received", lr=0.1)
    policy.update(state="task_received", action="verify_budget", reward=2.0, next_state="budget_ok", lr=0.1)
    ...
    assert deserialized.q_table["idle"]["fetch_task"] == policy.q_table["idle"]["fetch_task"]
...
async def test_mission_lock_agent_reward_shaping():
    ...
    conforming_reward = agent.shaped_reward(state="idle", action="fetch_task", base_reward=1.0)
    assert conforming_reward == 2.0
    
    # Deviating action: reward = base_reward - off_path_penalty = 1.0 - 0.5 = 0.5
    deviating_reward = agent.shaped_reward(state="idle", action="idle_action", base_reward=1.0)
    assert deviating_reward == 0.5
    
    # Forbidden action: reward = base_reward - off_path_penalty - 10 * safety_weight = 1.0 - 0.5 - 10.0 = -9.5
    forbidden_reward = agent.shaped_reward(state="idle", action="exceed_budget", base_reward=1.0)
...
async def test_stateless_service_save_and_load_agent_state(async_session):
    ...
    # Load agent
    agent = await MissionLockService.load_agent_state(agent_id=agent_id, actions=actions, db=async_session)
    ...
    reloaded_agent = await MissionLockService.load_agent_state(agent_id=agent_id, actions=actions, db=async_session)
...
async def test_continuous_authz_gate(async_session):
    ...
    granted, reason = await MissionLockService.continuous_authz_gate(
        user_id=user_id, tenant_id=tenant_id, action="WRITE", resource="dna_update", db=async_session
    )
    assert granted
    ...
    granted_read, reason_read = await MissionLockService.continuous_authz_gate(
        user_id=analyst_user, tenant_id=tenant_id, action="READ", resource="dna_view", db=async_session
    )
    assert granted_read
    
    # 4. ANALYST should fail WRITE
    granted_write, reason_write = await MissionLockService.continuous_authz_gate(
        user_id=analyst_user, tenant_id=tenant_id, action="WRITE", resource="dna_update", db=async_session
    )
...
async def test_idempotency_key_caching(async_session):
    ...
    # Save key
    await MissionLockService.save_idempotency(key=key, response=response_payload, db=async_session)
    await async_session.commit()
...
async def test_compile_time_gpc_constraint_extraction_and_registration(async_session):
    ...
    assert "exfiltrate_db" in dna.forbidden_actions  # Extracted from pipeline description text
    
    # 2. Assert Conformance Path exists and is active
    stmt_mission = select(AgentMission).where(AgentMission.agent_id == pipeline_id, AgentMission.active == True)
    res_mission = await async_session.execute(stmt_mission)
    mission = res_mission.scalar_one_or_none()
    
    assert mission is not None
    assert mission.preferred_transitions["idle"] == "check_auth"  # Extracted from description text "idle -> check_auth"
    assert mission.preferred_transitions["processing"] == "encrypt"  # Extracted from node configuration
...
async def test_vnp_governance_scoring_model(async_session):
    ...
    score = await update_agent_governance_score(session=async_session, agent_id=agent_id)
    ...
    score_degraded = await update_agent_governance_score(session=async_session, agent_id=agent_id)
