...
class GovernanceEngine:
    ...
    async def runGovernedExecution(
        ...
        try:
            ...
            try:
                ...
                ledger_event_pgl = await pgl_client.create_ledger_event(
                ...
                pgl_identity = {
                    "ledger_event_id": ledger_event_pgl.get("id"),
                    "agent_id": str(agent_id),
            ...
            # Audit failure to GnomLedger
            await cls._audit_rejection(agent_id, trace_id, he, request_body)
            raise he
        except Exception as e:
            # Audit unexpected failure to GnomLedger
            logger.exception(f"Unexpected error in governed execution: {e}")
            he = HTTPException(status_code=500, detail=str(e))
            await cls._audit_rejection(agent_id, trace_id, he, request_body)
            raise he
    ...
    async def _audit_rejection(
        ...
        """Audit rejection/failure to GnomLedger."""
        try:
            pgl_client = PGLClient()
            await pgl_client.create_ledger_event(
                agent_id=str(agent_id),
                event_type="violation",
                actor=request_body.get("user_id", "system"),
                summary=f"Governed execution REJECTED: {exc.status_code}",
                details={
                    "trace_id": trace_id,
                    "status_code": exc.status_code,
                    "reason": exc.detail,
                    "workspace_id": request_body.get("workspace_id", "default"),
                    "task_type": request_body.get("task_type", "unknown")
                }
            )
        except Exception as e:
            logger.warning(f"Failed to audit rejection to GnomLedger: {e}")
