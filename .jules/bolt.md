...
## 2025-02-28 - [Eliminate O(N) memory anti-patterns in backend DB aggregations]
**Learning:** Loading entire database rows into Python application memory using `len(result.scalars().all())` or manually iterating over rows to group counts creates serious O(N) memory bottlenecks and bandwidth waste. This is especially true for endpoints like `admin_billing.py` querying webhook dead letters or PGL identity gates counting attestations.
**Action:** Push counting and grouping logic down to the database level by utilizing SQLAlchemy's `select(func.count(model.id))` and `.group_by(model.status)`. This returns scalar values, dropping the application memory footprint to O(1) and significantly speeding up the operations.
